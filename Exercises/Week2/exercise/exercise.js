...
console.log(arr.filter(x => isEven(x)))
...
function multiply(...nbrs) {
    return nbrs.reduce((previous, current) => {
...
console.log(arr2.filter(divisibleBy(3)))
...
function colorCycle(arr_col = COLOR_CYCLE_DEFAULT) {
    var n = arr_col.length;
    var i = -1;

    return function give_color() {
        i = (i+1)%n;
...
console.log(randomInRange(12, 178));
...
function normalizeCounts(dct) {
    var values = Object.values(dct);

    var tot = values.reduce((x, y) => x + y);

    var normed_dct = new Object();

    Object.keys(dct).forEach(function(key) {
        normed_dct[key] = dct[key]/tot;
    });
...
setCharacterCountingFunction(countOccurencesNormalized, colorCycle(['red', 'orange', 'blue']));
/*
## Throwing balls
...
function simulateBall(v0, angle, num_steps=256, dt=0.05, g=-9.81) {
    ...
    var vx = v0*Math.cos(angle_rad);
    var vy = v0*Math.sin(angle_rad);

    var times = range(num_steps).map(x => x*dt);

    var positions = times.map(t => [vx*t, vy*t + 0.5*g*t*t]);

    positions = positions.filter(a => a[1] >= 0);
